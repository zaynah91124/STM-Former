...
def visualize(image_3d,modal_idx,vis_name):
    # image_3d T,Z,H,W
    image_vis = image_3d[modal_idx][7]
    image_vis = (image_vis-image_vis.min())/(image_vis.max()-image_vis.min())*255
    image_vis = image_vis[:,:,np.newaxis]
    cv2.imwrite('./vis/%s.jpg'%vis_name,image_vis)
...
class MultiPhaseLiverDataset(torch.utils.data.Dataset):
    def __init__(self, args, is_training=True):
        ...
        self.benign_malignant_dict = {0:0,1:1,2:0,3:1,4:0,5:0,6:1}
        ...
        if self.args.handcraft_feature:
            handcraft_csv_root = os.path.join('./preprocess/features_total.csv')
            self.handcraft_csv = pd.read_csv(handcraft_csv_root)
            ma = self.handcraft_csv.iloc[:,1:].max().to_numpy()
            mi = self.handcraft_csv.iloc[:,1:].min().to_numpy()

            np_data = self.handcraft_csv.iloc[:,1:].to_numpy()

            self.np_normalized_data = (np_data-mi)/((ma-mi)+1e-9)
            

            self.np_normalized_data = (self.np_normalized_data-0.5)/0.5
    ...
    def __getitem__(self, index):
        ...
        benign_malignant_label = self.benign_malignant_dict[label]
        ...
        if self.args.handcraft_feature:
            handcraft_feature = self.load_handcraft_feature(self.img_list[index])
            handcraft_feature = handcraft_feature.astype(np.float32)
            return {"images":image,"labels":label,"patient_id":patient_id,"handcraft_feature":handcraft_feature,"benign_malignant_labels":benign_malignant_label}
        
        return {"images":image,"labels":label,"patient_id":patient_id,"benign_malignant_labels":benign_malignant_label}
    ...
    def mixup(self, image, label):
        ...
        image_combine = lam * image + (1 - lam) * image_bal
    ...
    def load_handcraft_feature(self,mp_img_list):
        mp_handcraft_feature = []
        for img in mp_img_list:
            patient_id = img.split('/')[-2]
            modal = img.split('/')[-1].split('.')[0]
            cur_name = patient_id+'_'+modal
            target_row = self.handcraft_csv[self.handcraft_csv['ImageName']==cur_name]
            np_data = self.np_normalized_data[target_row.index]
            mp_handcraft_feature.append(np_data)
        mp_handcraft_feature = np.concatenate(mp_handcraft_feature,axis=0)
        return mp_handcraft_feature
    ...
    def transforms(self, mp_image, transform_list):
        ...
        if 'mask' in transform_list:
            mp_image = mask(mp_image)
        if 'sharpen' in transform_list:
            mp_image = sharpen(mp_image)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--train_anno_file', default='', type=str)
    parser.add_argument(
        '--val_anno_file', default='', type=str)
    ...
    parser.add_argument('--img_size', default=[(32, 256, 256),(16,128,128),(8,64,64)],
                        type=int, nargs='+', help='input image size.')
    parser.add_argument('--crop_size', default=[(28, 224, 224),(14,112,112),(6,56,56)],
