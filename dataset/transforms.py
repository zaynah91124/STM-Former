...
def resize2D(image,size,mode,input_type='chw'):
    image = image.astype(np.float32)
    if mode == 'bilinear':
        if input_type == 'chw':
        ...
        x = F.interpolate(image,size = size, mode = 'bilinear', align_corners=False)
        return x.cpu().numpy()
    elif mode == 'bicubic':
        if input_type == 'chw':
            image = torch.from_numpy(image).permute(2,0,1).unsqueeze(0)
            ...
            image = torch.from_numpy(image).unsqueeze(0)
        x = F.interpolate(image,size = size, mode = 'bicubic', align_corners=False)
        return x.cpu().numpy()
...
def random_crop_2d(image_phase_list,crop_shape):
    for phase_idx in range(len(image_phase_list)):
        y_shape,x_shape = image_phase_list[phase_idx].shape[-2:]
        y_min = np.random.randint(0, y_shape - crop_shape[0])
        x_min = np.random.randint(0, x_shape - crop_shape[1])
        image_phase_list[phase_idx] = image_phase_list[phase_idx][..., y_min:y_min+crop_shape[0], x_min:x_min+crop_shape[1]]
    
    return image_phase_list
...
def center_crop_2d(image_phase_list,target_shape):
    for phase_idx in range(len(image_phase_list)):
        y_shape,x_shape = image_phase_list[phase_idx].shape[-2:]
        y_min = y_shape // 2 - target_shape[0] // 2
        x_min = x_shape // 2 - target_shape[1] // 2
        image_phase_list[phase_idx] = image_phase_list[phase_idx][..., y_min:y_min+target_shape[0], x_min:x_min+target_shape[1]]
...
def randomflip_x_2d(image, p=0.5):
    if random.random() > p:
        return image
    else:
        return image[..., ::-1]

def randomflip_y_2d(image, p=0.5):
    if random.random() > p:
        return image
    else:
        return image[:, :, ::-1, ...]

def random_flip_2d(image_phase_list, mode='x', p=0.5):
    for phase_idx in range(len(image_phase_list)):

        if mode == 'x':
            image_phase_list[phase_idx] = randomflip_x(image_phase_list[phase_idx], p=p)
        elif mode == 'y':
            image_phase_list[phase_idx] = randomflip_y(image_phase_list[phase_idx], p=p)
        else:
            raise NotImplementedError(f'Unknown flip mode ({mode})')
...
def rotate_2d(image_phase_list,angle=10):
    angle_random = random.randint(-angle, angle)
    r_image_list = []
    for phase_idx in range(len(image_phase_list)):
        r_image = ndimage.rotate(image_phase_list[phase_idx], angle=angle_random, axes=(-2, -1), reshape=True)
        r_image_list.append(r_image)
    
    r_image_list = center_crop_2d(r_image_list,image_phase_list[0].shape[-2:])
...
def edge_2d(image_phase_list):
    ...
    for phase_idx in range(len(image_phase_list)):
        phase_image_list = []
        for img in image_phase_list[phase_idx]:
            img = ((img-img.min())/(img.max()-img.min())*255).transpose(1,2,0)
            gray_img = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
            sobelx = cv2.Sobel(gray_img, cv2.CV_64F, 1, 0, ksize=3,borderType=edge_mode)
            sobely = cv2.Sobel(gray_img, cv2.CV_64F, 0, 1, ksize=3,borderType=edge_mode)
            gradient_img = np.sqrt(sobelx**2 + sobely**2)
            gradient_img = np.expand_dims(gradient_img,axis=-1)
            gradient_img = np.uint8(gradient_img)
            bgr_img = cv2.cvtColor(gradient_img,cv2.COLOR_GRAY2BGR)
            bgr_img = (bgr_img/255.).transpose(2,0,1)
            phase_image_list.append(bgr_img)
        phase_image_np = np.stack(phase_image_list,axis=0)
        phase_image_np = phase_image_np.astype(np.float32)
        sobel_image_list.append(phase_image_np)

    return sobel_image_list
...
def emboss_2d(image_phase_list):
    emboss_list = []
    for phase_idx in range(len(image_phase_list)):
        phase_image_list = []
        for img in image_phase_list[phase_idx]:
            img = ((img-img.min())/(img.max()-img.min())*255).transpose(1,2,0)
            img_emboss = Image.fromarray(np.uint8(img))
            img_emboss=img_emboss.filter(ImageFilter.EMBOSS)
            img_emboss = (np.asarray(img_emboss)/255.).transpose(2,0,1)
            phase_image_list.append(img_emboss)
        phase_image_np = np.stack(phase_image_list,axis=0)
        phase_image_np = phase_image_np.astype(np.float32)
        emboss_list.append(phase_image_np)
