...
def main():
    ...
    if args.smoothing:
        if args.bce_loss:
            ...
            train_loss_fn = ClassBalancedLoss(samples_per_class=sample,num_classes=args.num_classes,is_smoothing=True,weights_factors=torch.tensor([1,1,1,1,1,1,1]))
        else:
            train_loss_fn = LabelSmoothingCrossEntropy(smoothing=args.smoothing)
    else:
        if args.cb_loss:
            sample = torch.tensor(count)
            train_loss_fn = ClassBalancedLoss(samples_per_class=sample,num_classes=args.num_classes,weights_factors=torch.tensor([1,1,1,1,1,1,1]))
        else:
...
def train_one_epoch(
    ...
    for batch_idx, item in enumerate(loader):
        ...
        with amp_autocast():
            if args.model == "uniformer_small_original" or args.model == "uniformer_base_original" \
            or args.model == "uniformer_xs_original" or args.model == "uniformer_xxs_original" or args.model == "two_branch_uniformer" or args.model == "three_branch_uniformer" \
            or args.model == "multiphase_Bilstm_model" or args.model == "eight_branch_uniformer" \
            or args.model == "multiphase_Bilstm_model_one_branch" or args.model == "multiphase_Bilstm_model_two_branch" \
            or args.model == "multiphase_Bilstm_model_four_branch" or args.model == "latent_fusion_uniformer_base" or args.model == "lstm_uniformer_base" \
            or args.model == "latent_fusion_lstm_uniformer_base" or args.model == "latent_fusion_cls_uniformer_base" or args.model == "latent_fusion_cls_uniformer_v2_base":
                output,visualizations = model(input,handcraft_input)
                loss = loss_fn(output, target)
            elif args.model == "multiphase_Bilstm_model_2cls_aug":
                output,visualizations,aug_cls = model(input,handcraft_input)
                loss_1 = loss_fn(output, target)
                loss_2 = loss_fn(aug_cls,benign_malignant_target)
                loss = 0.9*loss_1+0.1*loss_2
            elif args.model == "resnet3d_50_classify" or args.model == "swin_transformer_3d_base" or args.model == "inception_resnet_v2_classify":
                output = model(input)
                loss = loss_fn(output, target)
            else:
